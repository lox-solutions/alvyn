...
Choose `every` from measured replay latency, payload size, and write/storage cost. There is no universal event-count threshold such as 200 events.
...
const txResult = await BankAccountBalance.load(eventStore, accountId, {
  client,
});
```
...
## Encrypt Sensitive Snapshot State

Snapshot encryption is **not inherited** from an aggregate or its source events. Snapshot maintenance replays decrypted data, so an unencrypted snapshot can persist PII in plaintext even when every source event is encrypted. Configure every sensitive path in the snapshot state explicitly:

```typescript
import { defineSnapshot } from "@lox-solutions/alvyn";
...
const UserProfile = defineSnapshot<
  { name: string; email: string },
  UserEvents
>()({
  streamPrefix: "User",
  snapshotName: "UserProfile",
  every: 50,
...
Register `UserProfile` in `EventStore.snapshots`, configure secrets, and create the same `user:${entityId}` key used for the source events before writing. Revoking that key tombstones the encrypted snapshot as well. When the latest snapshot is tombstoned, loading falls back to `initialState` and replays source history with null-safe handlers. This does not erase earlier plaintext snapshots or downstream copies; address those separately.
...
Each generated snapshot appends a new row; previous snapshots remain in history. Storage grows with snapshot frequency and with the number of snapshot definitions per stream, rather than staying at one row per stream. `Aggregate.load()` still replays domain history; use the snapshot handle's `load()` to benefit from its cached state.
