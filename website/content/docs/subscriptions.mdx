...
This minimal endpoint serializes `globalPosition` as a decimal string because JSON cannot encode `bigint`. Before deployment, add authorization, cursor validation, error cleanup, and bounded backpressure handling. For implementation sketches and their operational limits, see the [Event Streaming & Resilient Consumer Playbook](/docs/playbooks/sse-and-consumer-scaling).
...
- The payload is the stored `data` column, with encrypted fields removed. Ciphertext lives separately in `encrypted_data` and is not returned by subscriptions.
- The consumer receives the payload in its stored schema format.
- Aggregate event-map types do not validate runtime payloads or restore encrypted fields. `eventTypes` filters also do not narrow the TypeScript union; check `event.type` before accessing event-specific fields.
...
| **Scale Behavior**     | Every replica receives every event          | One worker holds a row lock at a time; retries can republish |
