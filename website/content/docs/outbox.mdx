...
With Alvyn's Transactional Outbox, recording the intent to publish is atomic and durable; external delivery happens later:
    ...
    const processedCount = await eventStore.processOutbox(
      async (entries, client) => {
        for (const entry of entries) {
          ...
          await jsm.publish(
            entry.topic,
            new TextEncoder().encode(JSON.stringify(entry.payload)),
          );
        }
      },
      100,
...
- Each pending row is locked by **one worker at a time**, not necessarily processed only once over its lifetime.
...
An external publish may succeed before the worker crashes or the database commit fails. The next attempt can publish the same entry again. Wait for durable broker acknowledgement, retry failed batches, and deduplicate downstream using the outbox ID or CloudEvents `source` + `id` (scoped by destination). SQL writes using the supplied `client` can commit atomically with `processed_at`; external effects cannot.
...
| **Concurrency** | One worker locks a row at a time; publishes may repeat | Every replica/subscriber receives all events      |
...
For building query-optimized read models inside the same PostgreSQL database, see [Projections](/docs/projections). For architectural guidance on choosing between direct streaming and broker relays, see the [Event Streaming & Resilient Consumer Playbook](/docs/playbooks/sse-and-consumer-scaling).
