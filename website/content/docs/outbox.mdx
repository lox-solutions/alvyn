...
async function runOutboxRelay() {
  try {
    ...
    const processedCount = await eventStore.processOutbox(async (entries, client) => {
...
For building query-optimized read models inside the same PostgreSQL database, see [Projections](/docs/projections).
