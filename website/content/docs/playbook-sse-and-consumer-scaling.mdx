...
<Callout type="info">
  **Alvyn supports both models:** You can stream events directly via
  ...
  (`defineOutbox`) to relay events to Kafka, RabbitMQ, or NATS with guaranteed
...
export async function startResilientConsumer(sseUrl: string) {
  ...
  while (true) {
    ...
    console.log(
      `Connecting to ${sseUrl} (Last-Event-ID: ${lastEventId ?? "HEAD"})...`,
    );
...
export async function runActivePassiveConsumer(pool: Pool, sseUrl: string) {
  ...
  while (true) {
    ...
    try {
      ...
      if (acquired) {
        console.log(
          "Acquired leader lock! Starting background event stream...",
        );
...
export class HashPartitionedWorkerPool {
  ...
  public enqueue(streamId: string, task: () => Promise<void>): Promise<void> {
    ...
    const chain = this.queues[partition].then(task).catch((err) => {
      console.error(
        `Task error in partition ${partition} for ${streamId}:`,
        err,
      );
    });
