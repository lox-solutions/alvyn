...
<Callout type="info">
**PostgreSQL Connection Pooling (e.g., PgBouncer):** Because SSE clients maintain continuous streaming connections, ensure database proxies or pools operate in **Session Pooling** mode (or connect directly to PostgreSQL). **Transaction Pooling** does not retain session-level notifications (`LISTEN/NOTIFY`) or Advisory Locks.
</Callout>
...
export async function startResilientConsumer(sseUrl: string) {
  ...
  while (true) {
    ...
    try {
      ...
      console.error(`SSE connection dropped. Retrying in ${sleepMs}ms...`, error);
...
<Callout type="info">
**Production Parser Tip:** For production edge cases involving multi-line formatted JSON strings spanning multiple `data:` lines or custom SSE comments, consider using a lightweight, zero-dependency W3C parser.
</Callout>
...
// Tip: Derive a deterministic 64-bit lock key from your service name in PostgreSQL
// using `SELECT hashtext('my-service-worker')::bigint;` or configure a unique integer per worker.
export async function runActivePassiveConsumer(pool: Pool, sseUrl: string) {
  const LOCK_KEY = 772091; // Unique 64-bit integer identifier for your service background worker
...
<Callout type="info">
**Outbox Table Retention & Purging:** In high-throughput architectures, purge published outbox rows periodically (e.g., via a daily scheduled cron: `DELETE FROM alvyn_outbox WHERE published_at < NOW() - INTERVAL '7 days'`) to maintain compact table sizes and fast index lookups without touching historical events in the core event store.
</Callout>
