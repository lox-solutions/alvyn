...
```typescript
interface SnapshotDefinition<TState, TEvents extends EventMap> {
  streamPrefix: string;
  snapshotName: string;
  every: number;
  initialState: TState;
...
type AggregateStoredEvent<TEvents extends EventMap> = {
  [K in keyof TEvents & string]: StoredEvent<TEvents[K]> & { type: K };
}[keyof TEvents & string];

type AggregateReplayedEvent<TEvents extends EventMap> =
