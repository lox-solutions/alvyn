...
Loads all events for a stream from version 1. Handles decryption and upcasting automatically. Accepts an optional `maxEvents` number or an options object `{ maxEvents?: number; client?: PoolClient }` to read over an active transaction client.
