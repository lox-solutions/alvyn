...
| `secrets`       | `CryptoSecret[]`            | No       | Ordered versioned secrets; first encrypts, remaining decrypt         |
...
`secrets` can also be configured with `GDPR_CRYPTO_SECRETS=version:value,...`.
For example, `GDPR_CRYPTO_SECRETS=2:new-secret-base64,1:old-secret-base64`.
The first secret is used for new encryption, while later entries are
decryption-only. Keep old entries during rotation; no database migration or
downtime is required, and entity keys are lazily re-wrapped on their next
write. Versions must be unsigned 32-bit integers. Use a higher version for each
new secret; gaps are allowed. In an HA deployment, first deploy
`[old, new]` to every replica, then deploy `[new, old]`. See
...
**Throws:** `CryptoSecretsRequiredError` if no `secrets` or `GDPR_CRYPTO_SECRETS` was configured.
...
Thrown when crypto operations are attempted but no versioned crypto secrets were provided.
