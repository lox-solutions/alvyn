...
description: PostgreSQL table structure for the event store — events, event-backed snapshots, outbox, crypto keys, and projections.
icon: Database
---
...
| ----------------- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `global_position` | `BIGSERIAL PK`         | Unique sequence position; may have gaps and is not commit order                                                                     |
...
`{schema}` is a documentation placeholder, not executable SQL. The table above shows the final shape; setup adds `txid` via `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` for existing installations. The `crypto_key_id` and `event_global_pos` references are logical references, not declared foreign-key constraints.
...
Deduplication table for idempotent stream appends. Prevents duplicate event writes and repeated outbox notifications on retries.
...
| Column             | Type                   | Description                                                |
| ------------------ | ---------------------- | ---------------------------------------------------------- |
| `key`              | `TEXT PK`              | Idempotency key supplied by the caller                     |
| `stream_id`        | `TEXT NOT NULL`        | Target stream identifier                                   |
| `request_hash`     | `TEXT NOT NULL`        | SHA-256 fingerprint of the appended event payload          |
| `from_version`     | `INTEGER NOT NULL`     | Stream version of the first event in the original append   |
| `to_version`       | `INTEGER NOT NULL`     | Stream version of the last event in the original append    |
| `global_positions` | `BIGINT[] NOT NULL`    | Global positions assigned to events in the original append |
| `created_at`       | `TIMESTAMPTZ NOT NULL` | Auto-set by database, used for age-based retention cleanup |
...
Migrations are run by calling `eventStore.setup()`. This executes `CREATE TABLE IF NOT EXISTS` and `CREATE INDEX IF NOT EXISTS` statements, making it safe to call on every application startup. Concurrent in-flight calls within the same process are automatically deduplicated.
