...
Events without a handler in `evolve` are silently skipped. This is not a semantic forward-compatibility guarantee: old readers may compute incomplete state or miss invariants. Deploy compatible readers before enabling new event types or payload versions.
...
| `loadEvents({ eventStore, entityId, maxEvents? })` | Loads typed replayed domain events            |
| `append(eventStore, input)`                        | Appends typed events to the stream            |
...
Loads the aggregate by replaying all events.
...
```typescript
import { OptimisticConcurrencyError } from "alvyn";
...
The first handler receives runtime `null`, including when the creation event is tombstoned. Supply non-sensitive defaults for every required state field; never assume a previous state exists. Erasure of previously materialized copies requires separate cleanup.

## Full Example with All Options

```typescript
import { defineAggregate, type Upcaster } from "alvyn";
...
type UserEvents = {
  ...
  UserRenamed: { name: string; updatedAt: string | null };
...
const userRenamedV1ToV2: Upcaster = {
  ...
  upcast(data: { name: string }) {
    return { name: data.name, updatedAt: null };
  },
};
