---
title: Projections
description: Build read-optimized query models from the global event stream with atomic checkpoints and transactional consistency within PostgreSQL.
icon: Eye
---
...
Each projection tracks its own independent checkpoint position in PostgreSQL and processes events in `globalPosition` order. Sequence allocation order is not transaction commit order; the commit-safe watermark holds back positions until earlier in-flight writes are resolved.
...
```typescript
import { defineProjection } from "alvyn";
...
```typescript
import type { Projection, StoredEvent } from "alvyn";
...
<Callout>
  ...
  PostgreSQL. If the worker crashes or a SQL query fails, the entire batch rolls
  back and the checkpoint remains unchanged.
</Callout>
...
- **External Effects:** HTTP calls, broker publishes, and writes through another connection are not atomic with the checkpoint and may repeat after failure. Use an outbox and downstream deduplication for these effects.
