---
title: Introduction
description: Alvyn is a production-grade event store for PostgreSQL with type-safe aggregates, event-backed snapshots, GDPR crypto-shredding, projections, transactional outbox, and schema evolution.
---
...
- **Idempotency** — Deduplicate retried appends by key, returning cached boundaries without duplicate events or outbox messages
...
- [Snapshots](/docs/snapshots) — Speed up expensive stream-derived calculations
- [Subscriptions](/docs/subscriptions) — Catch up and tail events for fan-out consumers
- [Transactional Outbox](/docs/outbox) — Relay events to message brokers with at-least-once delivery
- [Projections](/docs/projections) — Build read models from the global event stream
...
- [Full-stack Example](/docs/full-stack-example) — Apollo, GraphQL, and live banking updates
