...
<Callout type="info">
  **Production sizing note:** These results demonstrate that Alvyn reliably
  handles the tested daily workload under the benchmarked configuration. Because
  performance in production depends on infrastructure, network topology, and
  database sizing, always validate capacity limits, write contention, and
  PostgreSQL high availability against your specific operational requirements.
</Callout>
...
Run the commands from the repository root. Node.js 22.18.0 or newer, pnpm, and a
