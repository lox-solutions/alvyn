...
Every run is deterministic when the same configuration and `--seed` are used.
...
The load tests are intentionally separate from `pnpm test` because they seed
large histories and may run for several minutes or hours.
...
- unexpected error rate below `0.1%`.
...
The following result was produced by the default daily profile and saved as
`http-load-daily.json`.
...
| Reads succeeded            |                      `700,000 / 700,000` | Read path remained correct                 |
| Deposits succeeded         |                      `300,000 / 300,000` | Write path remained correct                |
| Unexpected errors          |                                      `0` | Below the `0.1%` SLO                       |
...
balance matched the expected value. This difference between source and stored
events is expected because snapshot events share the same event streams.

### Latency and SLOs

| Operation    |     `p50` |      `p95` |       `p99` |                 SLO |
| ------------ | --------: | ---------: | ----------: | ------------------: |
| Reads        | `1.40 ms` |  `3.91 ms` | `100.10 ms` |     `p95 <= 250 ms` |
| Deposits     | `7.47 ms` | `14.81 ms` | `212.72 ms` |     `p95 <= 500 ms` |
| All requests | `1.96 ms` | `11.41 ms` | `119.24 ms` | Error rate `< 0.1%` |

The phase-level SLOs also passed. Off-peak latency was higher than peak latency
for this run: read `p95` was `75.46 ms` and deposit `p95` was `184.66 ms`.
Overall percentiles are dominated by the larger peak phase, so phase-level
values should be reviewed when making capacity decisions.
...
- the read and deposit aggregate paths remain correct over one million requests;
